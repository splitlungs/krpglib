...
namespace KRPGLib
{
    ...
    public class KRPGSystem : ModSystem
    {
        ...
        public override void StartServerSide(ICoreServerAPI api)
        {
            ...
            api.ChatCommands.Create("krpg")
            .WithDescription("Manage all Kronos RPG mods.")
            .RequiresPrivilege(Privilege.chat)
            .RequiresPlayer()
            .HandleWith((args) =>
            {
                return TextCommandResult.Success(KRPGCmd.helpMessage);
            })
            #region Help
            .BeginSubCommand("help")
            .HandleWith((args) => { return TextCommandResult.Success(KRPGCmd.helpMessage); })
                .BeginSubCommand("player")
                .HandleWith((args) => { return TextCommandResult.Success(KRPGCmd.helpPlayerMessage); })
                .EndSubCommand()
                .BeginSubCommand("reload")
                .HandleWith((args) => { return TextCommandResult.Success(KRPGCmd.helpReloadMessage); })
                .EndSubCommand()
                .BeginSubCommand("version")
                .HandleWith((args) => { return TextCommandResult.Success(KRPGCmd.helpVersionMessage); })
                .EndSubCommand()
            .EndSubCommand()
            #endregion
            #region Player
            ...
            #endregion
            #region Version
            // Hardcoded for now because it doesn't change often. There's gotta be a better way
            .BeginSubCommand("version")
            .HandleWith((args) =>
            {
                string ver = null;
                ...
                if (args.ArgCount != 1)
                {
                    ...
                    for (int i = 0; i < KRPGCmd.allKRPGMods.Length; i++)
                    {
                        ...
                        if (ver != null)
                            verResults += ("\n" + KRPGCmd.allKRPGMods[i] + " version: " + ver);
                        ver = null;
                    }
                }
                else
                {
                    if (KRPGCmd.allKRPGMods.Contains(args[0]))
                        verResults = (KRPGCmd.allKRPGMods.IndexOf(args[0]) + " version: " + ver);
                }
